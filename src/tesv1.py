...
def generate_paths(matrix, buffer_size):
    ...
    for x in range(len(matrix)):
        for y in range(len(matrix[0])):
            ...
            paths.extend(generate_paths_from(matrix, buffer_size, x, y))
    return paths

def generate_paths_from(matrix, buffer_size, x, y):
    if buffer_size == 1:
        return [[(x, y)]]
    ...
    for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:  # right, down, left, up
        ...
        if 0 <= nx < len(matrix) and 0 <= ny < len(matrix[0]):
            for path in generate_paths_from(matrix, buffer_size - 1, nx, ny):
